...
# @channel.use(SchedulerSchema(timers.every_custom_seconds(15)))
# async def every_minute_speaking(app: Ariadne):
#     await app.sendFriendMessage(1072621520, MessageChain.create("我又来了"))
SetuFind = AlconnaString(
  ".leetcode <content>",
  "--usr_slug <*tags:str> #在群中发送 leetcode userslug 可查询个人资料（userslug为个人主页地址最后的唯一识别代码）",
  "--每日一题<*tags:str> #在群中发送 leetcode每日一题 可查询每日一题 ",
)
@channel.use(ListenerSchema( listening_events=[GroupMessage],inline_dispatchers=[AlconnaDispatcher(alconna=SetuFind, help_flag="reply")]))
